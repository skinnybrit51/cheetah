    ...
    return {
        select: function (recordIndex, columnIndex) {
            ...
            if (bottomPixelAdjustment > 0) {
                // only move if a new record is selected
            ...
            if (recordIndex !== view.selected.recordIndex) {
                if (!(recordIndex >= recordView.topIndex &&
                    recordIndex <= recordView.bottomIndex)) {
                    // if not in view
                    if (recordIndex < recordView.topIndex) {
                        // moving up
                        if (bottomPixelAdjustment === 0) {
            ...
            if (columnIndex < view.columnTopIndex) {
                ...
                view.columnBottomIndex - columnIndex === 1) {
            ...
            var height = view.scrollbarY.scroller.height();
            view.scrollbarY.scroller.css('top', height * view.selected.recordIndex);
